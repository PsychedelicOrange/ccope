...
void addStringLiteral(struct TOKENS *tokens, char *literal, size_t temp_index)
{
    struct TOKEN token = {
        ...
        .utoken = {.identifier = {.type = STRING_LITERAL, .iden_string = string_copy_c_style(literal, temp_index)}}};
    tokens->array[(tokens->index)++] = token;
}

// TODO: detect name or number , add error format for transpiler
void checkWordAndAddToken(struct Hashmap hashmap, struct TOKENS *tokens, char *tempword, size_t temp_index)
{
    ...
    if (mtoken == NULL_DEBUG)
    {
        ...
        struct TOKEN token;
        token.utoken.identifier.iden_string = string_copy_c_style(tempword, temp_index);
...
struct TOKENS lex(FILE *file, char *filename)
{
    struct cursor c = {.file = file, .col = 1, .line = 1};
            ...
            while ((c = getc(file)) != EOF)
            {
                ...
                if (string_started)
                {
                    ...
                    case DQUOTE:
                        ...
                        {
                            string_started = 0;
                            checkWordAndAddToken(hashmap, &tokens, temp_string, temp_string_index);
                        }
                        break;
                    default:
                ...
                case COMMA:
                    ...
                    {
                        tokens_add_singletoken(&tokens, COMMA, file, line, col);
                    }
                    break;
                case EQUAL:
                    if (word_started)
